...
int rtp_send_data(char* buf,int len,char dt, int sockfd);
...
void h264_send_data(char* data,int len,int datatype, int sockfd)
{
	NALU_t *n;
	...
	while(readcnt != len)
	{
		...
		if(n->len <= 1400)
		{
			...
			rtp_send_data(sendbuf,bytes,dt, sockfd);	
		}
